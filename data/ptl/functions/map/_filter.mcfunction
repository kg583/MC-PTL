...
execute if score $diff tmp matches 0 run data modify storage ptl:stdout Filtered append from storage ptl:stdin Map.Self[0]
execute unless score $diff tmp matches 0 run data modify storage ptl:stdout Rest append from storage ptl:stdin Map.Self[0]

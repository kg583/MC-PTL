...
execute store success score $success temp run data get storage ptl:stdin Vector.Other[0]
execute if score $success temp matches 1 run scoreboard players operation $current temp *= $other var

scoreboard players add $list.remove stdout 1
...
execute unless score $diff tmp matches 0 run data modify storage ptl:stdout List.Self append from storage ptl:stdin List.Self[0]
data remove storage ptl:stdin List.Self[0]
execute unless score $diff tmp matches 0 if data storage ptl:stdin List.Self[0] run function ptl:list/_remove

...
execute if score $current tmp <= $list.start stdin if score $current tmp >= $list.end stdin if score $mod tmp matches 0 run data modify ptl:stdout List.Splice append from storage ptl:stdin List.Self[-1]
